...
func (m *Model) View() string {
	...
	if m.paletteOpen {
		...
		overlay := lipgloss.Place(
			max(m.width, lipgloss.Width(view)),
			max(m.height, countLines(view)),
			lipgloss.Center,
			lipgloss.Center,
			palette,
		...
		return overlayView(view, overlay, max(m.width, lipgloss.Width(view)), max(m.height, countLines(view)))
	}
